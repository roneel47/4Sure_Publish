...
import { MongoClient, Db as MongoDb, FindOneAndUpdateOptions, Timestamp } from 'mongodb';
...
(async () => {
  ...
  try {
    ...
    db = client.db(DATABASE_NAME); // Assign to the outer `db`
    ...
    resolveDbConnection(db); // Resolve the promise with the `db` instance
  } catch (error) {
    console.error("Error connecting to MongoDB or setting up:", error);
    db = null;
    rejectDbConnection(error); // Reject the promise
  }
})();
...
export async function updateGameRoom(
  gameId: string,
  operationData: Partial<GameRoom> | any,
  ...
  if (isCreatingNew) {
    // When creating a new room, operationData is the full newRoomData
    // All initial fields go into $setOnInsert. $set should be empty for initial creation.
    updateOps.$setOnInsert = {
      gameId: gameId, // Ensure gameId from filter is used for $setOnInsert
      playerCount: operationData.playerCount,
      players: operationData.players,
      status: operationData.status,
      secretsSetCount: operationData.secretsSetCount,
      targetMap: operationData.targetMap,
      turn: operationData.turn,
      winner: operationData.winner,
    };
    updateOps.$set = {}; // Explicitly empty $set for new document creation
  } else {
    // For updates to an existing room, or if upsert happens to match an existing one
    // operationData contains MongoDB update operators (e.g., $set, $inc) or fields to be $set
    if (Object.keys(operationData).some(key => key.startsWith('$'))) {
      // operationData already contains operators
      updateOps = { ...operationData };
      // Ensure gameId is not in any $set operation
      if (updateOps.$set && updateOps.$set.gameId) {
        delete updateOps.$set.gameId;
      }
    } else {
      // operationData is a flat object of fields to set
      const { gameId: opGameIdToExclude, ...fieldsToModify } = operationData;
      updateOps.$set = fieldsToModify;
    }
    // $setOnInsert should be minimal or empty for updates, mainly for the filter.gameId
    updateOps.$setOnInsert = { gameId: gameId, ...(updateOps.$setOnInsert || {}) };
  }
  ...
  try {
    ...
    if (result && result.value) {
      ...
      console.error(`MongoDB: findOneAndUpdate for ${gameId} did not return a document (was null or undefined). Filter: ${JSON.stringify(filter)}, Update: ${JSON.stringify(updateOps)}`);
      // If it was an insert and didn't return, try to fetch it to confirm
      if (options.upsert) {
        console.log(`MongoDB: Re-fetching room ${gameId} after upsert did not return document.`);
    ...
    if (error.code === 40 && error.codeName === 'ConflictingUpdateOperators') {
        console.error("Detailed ConflictingUpdateOperators error. Offending operators might be in the update document:", JSON.stringify(updateOps, null, 2));
    }
    return null;
  }
}
  ...
  if (req.method === 'POST') {
    if (res.socket.server.io) {
      ...
      const io = new SocketIOServer(res.socket.server, {
        path: '/api/socketio_c', // Path matches client
      ...
      io.on('connection', async (socket: CustomSocket) => { 
        try {
          await dbConnectionPromise; 
          if (!db) {
            console.error(`MongoDB: DB instance is null for new socket connection ${socket.id} even after promise. Critical connection issue.`);
          ...
          try {
            ...
            if (!room) {
              ...
              const newRoomDataForCreation: GameRoom = { 
                gameId: gameId, // gameId is part of the document structure
              ...
              roomUpdateResult = await updateGameRoom(gameId, newRoomDataForCreation, true); // Pass isCreatingNew = true
              if (!roomUpdateResult) {
